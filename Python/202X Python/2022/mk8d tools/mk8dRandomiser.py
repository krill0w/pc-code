...
def randomiser():
	...
	while loopCtrl == 5:
		...
		if customCupQuery == 'N':
			print(
				"Press enter to continue to the tracks, and then again to reveal the next track: ")  # had to add the if statement so that it would only show up now because it wouldn't make sense otherwise
			input()
		if customCupQuery == 'N':
			print('\n' * 1000)
			for x in range(numOfTracks):
				trackRoll = choice(track)
				if trackRoll in duplicates: trackRoll = choice(
					track)  # removes duplicates by checking if they have already been added to the array, and then rerolling if it has
				duplicates.append(trackRoll)
				print(f"\nTrack {x + 1}\n{trackRoll}")
				if x + 1 != numOfTracks: input()
			loopCtrl = 4
		elif customCupQuery == 'Y':
			cupLengthQuery = input(
				"Enter the length of cup you would like to play [[S]ingle (4), [D]ouble (8)]: ").upper()
			if cupLengthQuery == 'S':
				...
				for i in range(len(CUSTOMsingleCups[0]) - 1): input(
